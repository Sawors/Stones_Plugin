...
public class StraightCannon implements SiegeUnit {
    ...
    StraightCannon(Block core, Location cannonend, Vector direction, StoneProjectile projectile) {
        this.core = core;
        this.cannonend = cannonend;
        this.direction = direction;
        this.projectile = projectile;
    }
    public StraightCannon(Block core) {
        create(core);
    }
    StraightCannon(Block core, Vector direction) {
        create(core, direction);
    }
    StraightCannon() {
    }
    @Override
    public void create(Block core)  throws IndexOutOfBoundsException {
        ...
        throw new IndexOutOfBoundsException("Cannon could not be found on Block "+core);
    }
    
    public void create(Block core, Vector direction) throws IndexOutOfBoundsException {
        if(core.getType().equals(Material.DISPENSER) && ((Dispenser) core.getState()).getInventory().getItem(4) != null){
            Block cannon = core.getRelative(BlockFace.NORTH);
            Location cannonend = cannon.getLocation();
            Vector dirunit = new Vector(0,0,1);
            float power = 1;
            int cannonlength = 0;
            dirunit = direction.clone().multiply(1/direction.length());
                if(core.getLocation().clone().add(.5,.5,.5).add(dirunit).getBlock().getType().equals(Material.POLISHED_BASALT)){
                    for(int i2 = 1; i2 <= 6; i2++){
                        cannonend = core.getLocation().add(.5,.5,.5).add(dirunit.clone().multiply(i2));
                        if(cannonend.getBlock().getType().isAir()){
    ...
    public boolean check() {
        return core.getType().equals(Material.DISPENSER) && direction.length() > 0 && cannonend != null && cannonend.distance(core.getLocation().add(.5, .5, .5)) > .5;
    }
